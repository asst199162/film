...
export function onModalOpen(e) {
  ...
  movie = JSON.parse(localStorage.getItem("searchedMovies")).find(movie => movie.id === Number(id));
  ...
  if (JSON.parse(localStorage.getItem("watchedCard"))?.find(movie => movie.id === Number(id))) {
    btnRefs.addWatched.textContent = 'REMOVE FROM WATCHED';
    btnRefs.addWatched.classList.add('watched');
  }
  if (JSON.parse(localStorage.getItem("queuedCard"))?.find(movie => movie.id === Number(id))) {
...
function onBtnWatchedClick(e) {
  ...
  if (!e.target.classList.contains('watched')) {
    if (JSON.parse(localStorage.getItem("watchedCard"))) { watchedMovie = JSON.parse(localStorage.getItem("watchedCard")) };
    ...
    const filterWatchedMovie = watchedMovie.filter((item) => Number(item.id) !== Number(movie.id));
    console.log(filterWatchedMovie);
...
function onQueueBtnClick(e) {
  if (!e.target.classList.contains('queued')) {
    if (JSON.parse(localStorage.getItem("queuedCard"))) { watchedMovie = JSON.parse(localStorage.getItem("queuedCard")) };
    ...
    const filterQueuedMovie = queuedMovie.filter((item) => Number(item.id) !== Number(movie.id));
